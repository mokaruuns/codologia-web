...
3. Далее давайте посмотрим, в каком формате отправятся данные.
    ...
    4. В списке найдем запрос, который был отправлен на сервер.
    5. Нажмем на него и посмотрим, какие данные были отправлены на сервер.
       ![](img_5.png)
    6. Видим, что был отправлен POST запрос с нашими данными.
4. Измените данные в форме и отправьте их еще раз - посмотрите, изменился ли запрос.

## Домашнее задание

1. Перед этим заданием сделайте практическое задание 3, которое мы делали в классе - там добавились пояснения, они помогут вам сделать это задание.
2. Создать пустой сайт с формой.
3. Внутри формы должно находится 5 полей для ввода:
    1. 2 текстовых поля
    2. 1 поле для ввода пароля
    3. 1 поле для ввода email
    4. 1 кнопка для отправки данных
4. Форма должна отправлять данные на сайт ```https://yandex.ru``` методом ```POST```.
5. Сделать скриншот формы и данных(данные посмотреть через панель разработчика)
